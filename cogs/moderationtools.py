...
class moderation(commands.Cog):
    ...
    @nextcord.slash_command(name="softban", description="Bans and unbans a user so their messages may be deleted", guild_ids=[testServerId])
    @application_checks.has_any_role(1003142166794747965)
    async def softban(self, interaction:Interaction, *, member:nextcord.Member):
        user = await client.fetch_user(interaction.id)
        await member.send(f"You have been softbanned in order to delete your messages, please rejoin:\nhttps://discord.gg/viktovia")
